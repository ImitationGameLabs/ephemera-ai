...
use crate::models::{OnlineStatus, UserCredentials};
        ...
        Ok(user) => {
            ...
            match user_manager.update_heartbeat(&user.name).await {
                Ok(updated_user) => {
                    ...
                    if let Ok(Some(latest_message_id)) =
                        message_manager.get_latest_message_id().await
                    {
                        // Update user's message height to the latest message ID
                        let _ = user_manager
                            .update_message_height(&user.name, latest_message_id)
                            .await;
                    }

                    let response =
                        OnlineStatus { online: true, last_seen: updated_user.status.last_seen };
