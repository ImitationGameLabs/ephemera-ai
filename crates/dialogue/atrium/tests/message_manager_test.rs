...
async fn test_message_query() {
    ...
    for name in ["alice", "bob"] {
        user_manager.create_user(&CreateUserDto {
            name: name.to_string(),
            bio: "".to_string(),
            password: "pass".to_string(),
        }).await.unwrap();
    ...
    // 空列表
    let messages = message_manager.get_messages(None, None, None).await.unwrap();
    assert!(messages.is_empty());

    // 创建多条消息（alice 3条，bob 2条）
    for i in 0..3 {
        message_manager.create_message(&CreateMessageDto {
    ...
    // sender 过滤
    let alice_messages = message_manager.get_messages(Some("alice"), None, None).await.unwrap();
    assert_eq!(alice_messages.len(), 3);
    assert!(alice_messages.iter().all(|m| m.sender == "alice"));

    // 分页
    let first_page = message_manager.get_messages(None, Some(3), Some(0)).await.unwrap();
    assert_eq!(first_page.len(), 3);

    let second_page = message_manager.get_messages(None, Some(3), Some(3)).await.unwrap();
...
async fn test_message_incremental_sync() {
    ...
    let mut ids = Vec::new();
    for i in 0..5 {
        let msg = message_manager.create_message(&CreateMessageDto {
    ...
    // since_id: 获取第二条之后的消息
    let new_messages = message_manager.get_messages_since_id(ids[1], None).await.unwrap();
    ...
    // since_id 带 limit
    let limited = message_manager.get_messages_since_id(ids[1], Some(2)).await.unwrap();
    assert_eq!(limited.len(), 2);

    // since_id 为最新 id 时返回空
    let empty = message_manager.get_messages_since_id(ids[4], None).await.unwrap();
