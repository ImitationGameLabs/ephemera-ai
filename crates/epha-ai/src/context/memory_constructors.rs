...
/// Create a builder for a memory fragment from an Agora event
///
/// Dual timestamp design:
/// - content.original_timestamp: When event was created (Herald-side)
/// - MemoryFragment.timestamp: When event entered context (epha-ai side, generated by .build())
pub fn from_agora_event(event: Event) -> MemoryFragmentBuilder {
    let content = json!({
        "type": "agora_event",
        "event_type": event.event_type,
        "herald_id": event.herald_id,
        "payload": event.payload,
        "original_timestamp": serde_json::to_string(&event.timestamp).unwrap_or_else(|_| "\"unknown\"".to_string()),
